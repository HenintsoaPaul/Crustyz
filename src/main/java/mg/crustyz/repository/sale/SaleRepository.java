...
public interface SaleRepository extends JpaRepository<Sale, Integer> {
    ...
    @Query(value = "SELECT m FROM Sale m WHERE " +
           "( :dateMin IS NULL OR m.daty >= :dateMin ) AND " +
           "( :dateMax IS NULL OR m.daty <= :dateMax )", nativeQuery = true)
	List<Sale> findAllSalesInRange(LocalDate dateMin, LocalDate dateMax);
}
